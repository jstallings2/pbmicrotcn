...
class Base(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # pass the input thrgouh the mode
        # print('input.shape = ', input.shape, 'params.shape = ', params.shape)
        pred = self(input, params)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        for rand_idx in list(rand_indices):
            ...
            # log audio examples
            self.logger.experiment.add_audio(f"input/{rand_idx}",  
                                             i, self.global_step, 
                                             sample_rate=self.hparams.sample_rate)
            self.logger.experiment.add_audio(f"target/{rand_idx}", 
                                             t, self.global_step, 
                                             sample_rate=self.hparams.sample_rate)
            self.logger.experiment.add_audio(f"pred/{rand_idx}",   
                                             p, self.global_step, 
                                             sample_rate=self.hparams.sample_rate)
            ...
            if self.hparams.save_dir is not None:
                ...
                input_filename = os.path.join(self.hparams.save_dir, f"{rand_idx}-input-{int(prm[0]):1d}-{prm[1]:0.2f}.wav")
                target_filename = os.path.join(self.hparams.save_dir, f"{rand_idx}-target-{int(prm[0]):1d}-{prm[1]:0.2f}.wav")
                ...
                torchaudio.save(os.path.join(self.hparams.save_dir, 
                                f"{rand_idx}-pred-{self.hparams.train_loss}-{int(prm[0]):1d}-{prm[1]:0.2f}.wav"), 
