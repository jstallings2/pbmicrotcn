...
MODEL_ID = '1-uTCN-100__causal__4-10-5__fraction-1.0-bs32__pb_comp'
MODEL_VERSION = '2'

class _Compressor_Service():
...
def Compressor_Service():
    """Factory function for Compressor_Service class.
    :return _Compressor_Service._instance (_Compressor_Service):
    """

    # ensure an instance is created only the first time the factory function is called
    if _Compressor_Service._instance is None:
        _Compressor_Service._instance = _Compressor_Service()
        _Compressor_Service.model = _Compressor_Service.load_model(_Compressor_Service._instance, MODEL_DIR, MODEL_ID, MODEL_VERSION)
    return _Compressor_Service._instance

def compress(inputfile, ratio, threshold, gpu=False, verbose=False):
    compressor = Compressor_Service()

    return compressor.process(inputfile, ratio, threshold, gpu, verbose)

if __name__ == "__main__":

    # Gradio Interface
    # Inputs: Audio, Slider, Slider, Checkbox, Checkbox
    # Outputs: Audio
    iface = gr.Interface(fn=compress, 
                        inputs=[gr.inputs.Audio(type="filepath"),
                                gr.inputs.Radio(['2', '4', '8', '16', '32'], default='16'), 
                                gr.inputs.Slider(minimum=10, maximum=50, step=10, default=30),
                                "checkbox",
                                "checkbox"],
